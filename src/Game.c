...
void Game_updateState(Game *game) {
  ...
  Player_renderPoints(game->player1, game->renderer, 270);
  Player_renderPoints(game->player2, game->renderer, 410);

  Ball_handleCollision(game->ball, game->paddleLeft, game->paddleRight,
  ...
  Paddle_handlePaddlePositionEnemy(game->paddleRight, state);
...
void Game_run(Game *game) {
  ...
  while (running) {
    ...
    SDL_Delay(
        16); // Approximately 60 frames per second (1000ms/60 ≈ 16ms per frame)
  }
}
